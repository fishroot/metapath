...
class bioconductor:
    ...
    def convert_geneids(self, input_list = [],
        ...
        if input_format in annotation_packages:
            ...
            #self.logger.info("sending command to R: library('%s')" % (package_name))
            try:
                sys.stdout = NullDevice()
                self.r.library(package_name)
                sys.stdout = original_stdout
            except:
                sys.stdout = original_stdout
                self.logger.warning("could not find R/bioconductor package: '%s'" % (package_name))
                self.logger.warning("trying to install ...")
                self.install([package_name])
##                self.logger.critical("you have to install the R/bioconductor package: '%s'" % (package_name))
##                quit()
